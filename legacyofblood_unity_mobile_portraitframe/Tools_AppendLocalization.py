...
combo_mag_5=<color=#ff00ff>[Ấn Độc] nỗ tung trên {0} gây {1} ST CHUẨN!</color>
...
combo_arc_5=<color=#ffaa00>Phát Đạn Đoạt Mệnh thành công, {0} được đánh thêm 1 lượt!</color>
...
def safe_append(path, append_text):
    # Determine newline character from file if possible, or just append tightly
    with open(path, "a", encoding="utf-8") as f:
        f.write("\n" + append_text.strip() + "\n")

safe_append(vi_path, vi_text)
safe_append(en_path, en_text)
print("Safely appended text without touching original bytes using python's utf-8 append mode.")
